...
class ProductAdapter(val productList: List<Product>?) : RecyclerView.Adapter<ProductAdapter.ViewHolder>() {
    ...
    override fun onBindViewHolder(holder: ProductAdapter.ViewHolder, position: Int) {
        holder.text1.text = "PRODUCT ID: " + productList?.get(position)?.productId
        holder.text2.text = "NAME: " + productList?.get(position)?.name
        holder.text3.text = "DESCRIPTION: " + productList?.get(position)?.description
    }
